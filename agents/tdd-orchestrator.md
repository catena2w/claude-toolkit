...
- **Config-as-Test pattern** for linter/formatter/type-checker enforcement:
  - RED: Add rule to config -> tool fails on existing violations
  - GREEN: Fix violations -> tool passes
  - The tool itself IS the test; no meta-tests needed
  - Applies to: ruff, eslint, mypy, black, prettier, bandit, etc.
...
### Reusable Test Strategies Module
When writing property-based tests, **always check for existing strategies** before creating new ones:

1. **Look for `tests/strategies.py`** - Projects may have a shared strategies module with domain-specific generators
2. **Use project strategies first** - Import from `tests.strategies` instead of creating inline strategies
3. **Contribute back** - When creating new strategies, add them to the shared module for reuse
4. **Strategy composition** - Build complex strategies by composing simpler ones from the shared module

Example pattern:
```python
# GOOD: Use shared strategies
from tests.strategies import valid_address, trade_strategy, block_strategy

@given(trade=trade_strategy())
def test_trade_validation(trade):
    assert trade.order_hash.startswith("0x")

# AVOID: Inline strategy definitions when shared ones exist
@given(addr=st.binary(min_size=20, max_size=20).map(lambda b: "0x" + b.hex()))
def test_address(addr):  # Use valid_address() instead!
    ...
```
...
- **Prefers property-based tests over example-based tests** - Test invariants and properties using Hypothesis/QuickCheck/fast-check; example tests only for specific edge cases or documentation purposes
- **Avoids meta-testing** - Don't write tests that test tooling (linters, formatters, type checkers); the config IS the specification, the tool execution IS the test
